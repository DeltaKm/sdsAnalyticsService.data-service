...
import { getCatalogByTypeAnalytics } from "@/lib/analytics/catalog";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getCatalogByTypeAnalytics(parsed.data);
    const payload = analyticsResponseSchema.parse(data);
    return NextResponse.json(payload);
  } catch (error) {
    console.error("Catalog by-type analytics error", error);
    return NextResponse.json({ error: "Failed to load catalog by-type analytics" }, { status: 500 });
  }
}
