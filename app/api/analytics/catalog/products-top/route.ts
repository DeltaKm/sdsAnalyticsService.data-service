...
import { getCatalogProductsTopAnalytics } from "@/lib/analytics/catalog";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getCatalogProductsTopAnalytics(parsed.data);
    const payload = analyticsResponseSchema.parse(data);
    return NextResponse.json(payload);
  } catch (error) {
    console.error("Catalog products analytics error", error);
    return NextResponse.json({ error: "Failed to load catalog products analytics" }, { status: 500 });
  }
}
